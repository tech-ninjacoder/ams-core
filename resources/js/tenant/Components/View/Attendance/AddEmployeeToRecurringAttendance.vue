<template>
    <modal id="recurring-attendance-employee-modal"
           v-model="showModal"
    ...
    >
        <form
            :data-url='`${apiUrl.RECURRING_ATTENDANCE}/${id}/add-employees`'
            method="POST"
            ref="form"
        >
<!--            <app-note-->
...
export default {
    name: "AddEmployeeToRecurringAttendance",
    ...
    mounted() {
        this.preloader = true;
        this.getRecurringAttendanceUsers()
    },
    methods: {
        afterSuccess({data}) {
            this.formData = {users: []};
            $('#recurring-attendance-employee-modal').modal('hide')
            this.toastAndReload(data.message, 'recurring-attendance-table');
        },
        getRecurringAttendanceUsers() {
            axiosGet(`${this.apiUrl.RECURRING_ATTENDANCE}/${this.id}/users`).then(({data}) => {
