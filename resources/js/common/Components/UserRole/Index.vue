<template>
    <div class="content-wrapper">
        <app-page-top-section :title="$fieldTitle('users_roles')" icon="user-check">
            ...
            />
            <app-default-button btn-class="btn btn-success mr-2"
                                :title="$fieldTitle('add', 'admin', true)"
                                @click="isAdminInviterOpenModalActive = true"
                                v-if="$can('invite_user')"
            />
<!--            <app-default-button-->
<!--                :title="$addLabel('role')"-->
...
<!--            />-->
            <app-add-admin-modal
                    from="user"
                    v-if="isAdminInviterOpenModalActive "
                    v-model="isAdminInviterOpenModalActive"
                    :selected-url="''"
            />
        </div>
        <div v-else>
...
export default {
    ...
    data() {
        return {
            ...
            isAdminInviterOpenModalActive: false,
