    ...
    methods: {
        triggerActions(row, action, active) {
            ...
            if (action.name === 'cancel') {
                ...
                this.employeeListUpdate()
            } else if (action.name === 'add_to_guardians') {
              this.guardian_list_update_url =  `${this.selectedUrl}/add-to-guardians`
              this.guardianListUpdate()
            } else if (action.name === 'remove_from_guardians') {
              console.log('gariddd')
              this.guardian_list_update_url =  `${this.selectedUrl}/remove-from-guardians`
              this.guardianListUpdate()
            } else {
      ...
      guardianListUpdate(){
        axiosPatch(this.guardian_list_update_url)
            .then(({data}) =>{
              this.guardian_list_update_url= '';
              this.toastAndReload(data.message, 'user-table')
            }).catch(({response}) => this.toastException(response.data))
      },
        filterUser(id) {
