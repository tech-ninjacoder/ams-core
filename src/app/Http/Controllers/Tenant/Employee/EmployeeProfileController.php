...
class EmployeeProfileController extends Controller
{
    ...
    public function updateEmployees($type)
    {
        ...
        if ($type == 'joining-date') {
            ...
            $project = Project::query()->findOrFail(request()->get('project_id'));
            resolve(ProjectService::class)
                ->setModel($project)
                ->assignToUsers($users_id);
            return updated_responses('employees');
        }
        ...
        }else if ($type == 'clear-project'){
            ProjectUser::query()
                ->whereIn('user_id', $users_id)
                ->update(['end_date' => Carbon::today()->toDateString()]);
            return response()->json([
                'status' => true,
                'message' => __t('projects are cleared')
            ]);
        }else{
