...
class ProjectCoordinatorController extends Controller
{
    ...
    public function index(Project $project): array
    {
        ...
        $project = $project->load(['coordinators' => function ($builder) use ($DeptUsers) {
...
//            ->toArray();
        ...
        return $project->coordinators->pluck('id')->toArray();
    }
    ...
    public function store(Project $project, Request $request)
    {
        $this->service
            ->setIsUpdating(true)
            ->setAttribute('id',$request->get('coordinators'))
...
//            ->assignCoordinatorToUsers(request('access_behavior') == 'own_departments' ?
//                $this->service->mergeNonAccessibleUsers($request->get('coordinators', [])) :
//                request()->get('coordinators', [])
            ->assignCoordinatorToUsers(
                request()->get('coordinators', [])
            );
    ...
    public function update(Request $request)
    {
        ...
        DB::transaction(function() use($project, $request) {
            $this->service
                ->setModel($project)
                ->setAttributes($request->only('project_id', 'coordinators'))
                ->moveCoordinator();
