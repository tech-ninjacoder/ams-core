...
class RecurringAttendanceUserController extends Controller
{
    public function __construct(RecurringAttendanceService $service)
    {
        $this->service = $service;
    }

    public function index(RecurringAttendance $ra): array
    {
//        $DeptUsers = resolve(DepartmentRepository::class)->getDepartmentUsers(auth()->id());

//        $ra = $ra->load(['users' => function ($builder) use ($DeptUsers) {
...
//            ->toArray();
        ...
        return $ra->users->pluck('id')->toArray();
    }


    public function RaEmployees(RecurringAttendance $ra): array
    {
...
//        });
    ...
    public function store(RecurringAttendance $ra, Request $request)
    {
        $this->service
...
//                request()->get('users', []));
    ...
    public function update(Request $request)
    {
        ...
        $ra = RecurringAttendance::findOrFail($request->get('department_id'));

        DB::transaction(function() use($ra, $request) {
            $this->service
                ->setModel($ra)
                ->setAttributes($request->only('recurring_attendance_id', 'users'))
                ->moveEmployee();
