...
class EmployeeService extends TenantService
{
    ...
    public function update()
    {
        DB::transaction(function () {
            $this->model->fill($this->getAttributes('first_name', 'last_name', 'email','guardian_id'));
    ...
    {
        $this->when($this->getAttr('provider_id'), function (EmployeeService $service) {
            $service->manager
                ->walkInto('provider', $this->getAttr('provider_id'))
                ->assignEmployees($this->model->id);
    ...
    public function assignToHelmet()
    {
        $this->when($this->getAttr('helmet_id'), function (EmployeeService $service) {
            $service->manager
                ->walkInto('helmet', $this->getAttr('helmet_id'))
                ->assignEmployees($this->model->id);
            Log::info("helmet");
...
//    public function assignToGatePass()
//    {
//        $this->when($this->getAttr('gate_pass_id'), function (EmployeeService $service) {
//            $service->manager
//                ->walkInto('gate_pass', $this->getAttr('gate_pass_id'))
//                ->assignEmployees($this->model->id);
//            Log::info("Gate Pass");
//
//        });
...
//    }
    public function assignToGatePass()
    {
        $this->when($this->getAttr('gate_passes'), function (EmployeeService $service) {
    ...
    public function assignToSkills()
    {
        $this->when($this->getAttr('skills'), function (EmployeeService $service) {
            resolve(UserService::class)
                ->setModel($service->model)
                ->attachSkill();
        });
...
//    public function assignToSkill()
//    {
//        $this->when($this->getAttr('skills'), function (EmployeeService $service) {
//            $service->manager
//                ->walkInto('skill', $this->getAttr('skills'))
//                ->assignEmployees($this->model->id);
//            Log::info($this->getAttr('skills'));
//
//        });
//
//        return $this;
