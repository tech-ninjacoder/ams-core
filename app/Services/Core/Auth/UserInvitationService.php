...
class UserInvitationService extends BaseService
{
    ...
    public function invite($email,$employee_id,$first_name,$last_name, $roles = [])
    {
        $roles = count($roles) ? $roles : request()->get('roles');

        $this->create($email,$employee_id, $first_name,$last_name)->assignRoles($roles);
    ...
    public function create($email,$employee_id, $first_name, $last_name, array $attributes = [])
    {
